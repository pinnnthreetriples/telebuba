...
## State
- Working: React/FastAPI; accounts, sessions, proxy pool, profile media, channels, warming runtime, neurocomment listener and vision solver, automated campaign channel discovery (Telegram native search + Telemetr.io), strict CI with CVE gates on both dependency trees (`pip-audit` and `npm-audit`).
