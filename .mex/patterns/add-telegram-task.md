...
3. Implement Telethon dispatch in the owning `core/telegram_client/` submodule and add a `match` arm in `_actions.py` (writes) or `_read.py` (reads). Both have fall-throughs: a write whose `action_type` starts with `channel_` routes to `_channels._dispatch_channel_action`, and anything unmatched lands in `_dispatch_profile_media_action` — so a missing arm fails as a confusing media error, not a clean one.
