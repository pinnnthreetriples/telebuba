...
3. Delegate DB, Telegram, providers, config and logging to `core/` gateways. In a package, route the injectable collaborators (`execute`, `generate_text`, `_rng`, …) through the domain's `_seams.py` so tests patch one place instead of every submodule.
