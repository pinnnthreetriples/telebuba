...
- Global errors/warnings feed (same branch) — **Done**: the board's bottom «Живая активность» feed is repurposed into «Ошибки и предупреждения · по всем аккаунтам» — it now shows only WARNING/ERROR rows aggregated across every account (the success "what happened" stream lives per-card now). New `LogFilter.problems_only` flag → `core/repositories/logs.py` adds `where(status != "success")`; `features/warming/_activity.py` queries `problems_only=True` and drops the old warming-only toggle / prefix filter (net shorter). Errors already get logged via `log_event`, so nothing is "duplicated" — just filtered. Tested in `tests/services/test_logs.py`. 623 tests green; ruff/ty/aislop clean.
