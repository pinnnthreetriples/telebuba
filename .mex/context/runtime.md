...
- An avatar or bio that reached Telegram is NOT the same as one strangers can see: `account.getPrivacy`/`setPrivacy` on `ProfilePhoto` / `About` / `StatusTimestamp` decide that, and a contacts-only photo key is why a correctly uploaded avatar still renders as a letter placeholder to a non-contact while our own live read (`UserFull.profile_photo.id` → `is_main`) reports it present. `core/telegram_client/_privacy.py` owns both directions; the read collapses Telegram's rule vector onto ONE level, taking the base rule and ignoring per-user exceptions, and anything unmodelled (close friends, premium) becomes `unknown` — a level the SPA shows but can never send back, because guessing it as `everybody` would be a lie about who can see the account. `setPrivacy` REPLACES a key's whole vector, so exceptions are dropped by design. `set_privacy_settings` is deliberately NOT in `_PROFILE_EDIT_ACTION_TYPES`: it has a fleet-wide apply, and one press could park a large slice of the fleet in `flood_wait`, which blocks `start_warming` — refusals are already reported per account instead. The fleet sweep is a synchronous route at concurrency 4 (`services/accounts/privacy.py`, matching `neurocomment.ban_check_concurrency`), skips accounts whose `health_for_status` is not `ok`, and puts only an exception's CLASS name in its response (a message can carry a proxy host or a session path).
