...
- The operator captcha queue («Требуют проверки») lists only rows its «Повторить» button can act on: the retry erases readiness and re-onboards, so a pair that is banned, operator-skipped, or out of the chat with its re-join budget spent is HIDDEN, not badged — a row nobody can clear is a second list to triage. Rows also age out after `challenge_queue_max_age_days` (3), since the audit table keeps failures for the whole 90-day retention. Banned/skipped/age are SQL (`_challenges._retry_can_reach`, `since`); the budget is `_rejoin.access_lost` + `exhausted` subtracted in `campaigns.list_campaign_challenges`, because core cannot see `settings.neurocomment` or the terminal-verdict set. A pair still inside its budget stays listed — the sweep may put it back in the chat, and a kick after a give_up must not delete the give_up.
