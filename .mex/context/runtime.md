...
- The operator captcha queue («Требуют проверки») lists only rows whose «Повторить» button should be pressed: banned (#30), operator-skipped (#148), re-join budget spent, channel serving a #147 pause, or older than `challenge_queue_max_age_days` (3) are HIDDEN, not badged — a row nobody can clear is a second list to triage. **The reason is NOT that onboarding refuses them.** `challenge.retry_pair` DELETES the readiness row before onboarding, so every guard in `_join_and_classify` keyed on that row (skip/ban refusal, re-join back-off) is skipped by construction and the retry really does re-join and re-solve — that is why the rows must go: the click would lift a permanent ban, undo the operator's own skip, or spend a join RPC Telegram already refused four times while resetting `rejoin_attempts` to zero. Only the pause is genuinely unreachable (it returns before the join RPC). EVERY exclusion has to be inside the one SQL statement, because `limit` is applied by the database: the queue lists challenge ROWS and a pair collects one per onboarding pass, so six finished pairs are ~24 rows that filled a 20-row page and hid the one actionable pair. The budget stays defined once, in `_rejoin.access_lost` + `exhausted`, and reaches core as an `AccountChannel` exclusion list; a pair still inside its budget stays listed, and a kick after a give_up must not delete the give_up.
- The board's `bot_challenge` badge and the captcha queue read the same table and are ALLOWED to differ: the badge answers "why is this channel stuck", the queue "what can a human press today". What they may not disagree on is a fact — `list_challenged_channels` had no `_still_blocked`, so one long-resolved `give_up` (append-only table) made `_channel_status` answer `bot_challenge` for a channel Telegram was write-blocking, naming the wrong wall. It now shares that one exclusion and no others: applying the age window or the skip exclusion there would report `chat_restricted` for a channel a bot gate really is holding.
