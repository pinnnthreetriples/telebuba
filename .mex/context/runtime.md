...
- The pre-cycle reservation (#208) books the whole remaining daily budget and the hand-back reconciles it down. That hand-back is guarded by a PER-BOOKING TOKEN (`warming_account_state.reservation_token`, migration #46; `hand_back_warming_reservation` matches token + date + booked value and clears the token), never by `run_id`: the stop path clears the generation and writes `idle` the moment its ~5s cancel wait is up, and Start mints the next one eagerly, so any generation-based CAS refuses exactly the hand-back a slow-unwinding cycle owes — which cost the account the rest of the day on a routine Stop→Start. The booked NUMBER cannot stand in for the token: `booked` saturates to the phase cap for every generation (the daily gate admits only `count <= cap - 2`), so a value-only guard matched a newer generation's identical re-booking and released it (measured: `ABA: run-2's live booking of 15 was released to 4 by the dead run's retried hand-back`). The token is what keeps a dead cycle off a live booking and what makes the retry a no-op — `asyncio.shield` leaves a cancelled hand-back landing as a detached task that the loop's exit handler really does re-run (measured: two writes ~15 ms apart). Keep it an ABSOLUTE write: a relative give-back would double-subtract there and overspend the cap. Refusals are classified by `_classify_refusal` from the row's token/date/count, because only two of the four leaves cost budget: `settled` (token NULL — only our own applying write clears it; or our token above a count that already moved to the reconciled value, which `_finalize_after_cycle` does without touching the token, or above a date that rolled past midnight) is silent, while `absorbed` (someone else's token: a newer booking read its baseline off our un-reconciled booking and folded our remainder in — reachable when a phase advance raises the cap, so the gate admits the saturated count instead of parking) and `stranded` (our token, our date, count at or above `booked`; no known producer, kept fail-loud) both WARN, under SEPARATE codes — `warming_reservation_absorbed` and `warming_reservation_stranded` — because `absorbed` has a known false positive (see below) and `stranded` should never fire at all, so sharing one code would let the first dilute the second. Separate codes are also the only vocabulary the i18n drift guard reads: `_module_reason_codes` sees literals and `reason`-named locals, never a name bound to a call result, so an `extra["reason"]` carrying the classifier's verdict was guarded by nothing. Their payload names OUR booking (`spent`, `booked`, `unreleased`) and never the row's count: after a refusal that count can be a newer booking's. `warming_reservation_reconciled` fires only when something was actually handed back (`unreleased > 0`) — a cycle that spent its whole remaining budget and was then cancelled reaches the same applying write, and telling an account that merely hit its cap that it nearly lost a day is how the alarm gets ignored. A token surviving on the row does NOT mean the booking did — that assumption made a quarter of stops warn about days that were not lost, and its inverse hid a real one. A hard kill still forfeits the day by design (no handler ran, so the real spend is unknown) — that is the fail-closed direction, do not "recover" it by releasing a booking whose spend nobody witnessed. The token only has to survive in the row because nothing loads it: it is absent from `WarmingStateRecord`, and `WarmingStateWrite.reservation_token=None` means "leave the column alone", so the writes between a booking and its hand-back carry it without knowing it exists.
...
- Challenge handling distinguishes Telegram restrictions from bot challenges and supports configured OpenAI/Gemini text/vision solving, retries, caching, and operator actions. A lost challenge is now finished WITHOUT an operator (#49): `_captcha_retry` rides the 5-minute deletion sweep, authorises exactly ONE re-solve (`captcha_retry_at`, stamped before the poke so a pair the pass cannot reach still terminates), and on its failure marks `captcha_gave_up`, leaves the discussion group best-effort, and unlinks the channel once every serving account has gone the same way. The budget is a persisted stamp, NOT a count of `neurocomment_challenges` rows: that table counts every trigger's attempts (three operator Starts would finish a pair on the spot) and never moves for a pair the poke cannot reach. It identifies the pair instead — the readiness triple `(joined=1, captcha_passed=0, ready=0)` is also what the `_GATE_ERRORS` mute branch writes, and a failed challenge row inside the 48h window is the discriminator. The only channel-level pause left is `_channel_pause` (#147, K consecutive write failures); the deleted-comment back-off is gone — the sweep records deletions and nothing more.
...
- `DELETE /logs?event_prefix=…` (the operator "clear logs" button) is prefix-scoped and unbounded in time, and it used to record nothing: one press erased a month of `neurocomment_*` history while `debug.log` still held every event, and the silence read as a broken engine for days. It now writes a `logs_cleared` audit row AFTER the delete, under a code carrying no domain prefix so the next press cannot erase it, and skips the row when nothing was deleted. `GET /logs/count` answers the same clause the delete uses, so the number the operator confirms cannot disagree with the number that goes.
...
- The captcha queue («Сейчас решается») holds NO control since #49 — `retry_pair`, `POST /neurocomment/retry` and the «Повторить» button are all deleted — so a listed row is a claim that a pair is being worked on RIGHT NOW, and every exclusion keeps that claim true. Hidden, not badged: banned (#30), operator-skipped (#148), captcha given up (#49), re-join budget spent, channel serving a #147 pause, and rows older than `challenge_queue_max_age_days` (3). Nothing deletes a readiness row any more, which is what makes every terminal verdict in this domain one-way. EVERY exclusion has to be inside the one SQL statement, because `limit` is applied by the database: the queue lists challenge ROWS and a pair collects one per onboarding pass, so six finished pairs are ~24 rows that filled a 20-row page and hid the one actionable pair. The budget stays defined once, in `_rejoin.access_lost` + `exhausted`, and reaches core as an `AccountChannel` exclusion list; a pair still inside its budget stays listed, and a kick after a give_up must not delete the give_up.
- The board's `bot_challenge` badge and the captcha queue read the same table and are ALLOWED to differ: the badge answers "why is this channel stuck", the queue "who is the engine working on right now". What they may not disagree on is a fact — `list_challenged_channels` had no `_still_blocked`, so one long-resolved `give_up` (append-only table) made `_channel_status` answer `bot_challenge` for a channel Telegram was write-blocking, naming the wrong wall. It now shares that one exclusion and no others: applying the age window or the skip exclusion there would report `chat_restricted` for a channel a bot gate really is holding.
- Anti-freeze joins (#270): every channel-join site (campaign onboarding, the single-pair path, listener) paces with a jittered delay, breaks the burst on FloodWait/cooldown, and is gated by a persisted rolling-24h per-account cap (`neurocomment_join_log`, `max_joins_per_account_per_day`, default 20). `already_participant` no-op re-joins are a success but not counted. The cap counts NC joins only (warming joins uncounted → same-day carryover uncounted).
