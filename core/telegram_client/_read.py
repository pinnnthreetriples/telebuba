...
from core.telegram_client._read_channels import _dispatch_channel_read_action
from core.telegram_client._read_chat import (
    dispatch_read_chat_messages,
    dispatch_resolve_chat,
)
from core.telegram_client._read_comments import (
...
async def _dispatch_read_action(  # noqa: C901, PLR0911, PLR0912 - one return per read-action case
    ...
    match action:
        ...
        case ReadPostComments():
            return await dispatch_read_post_comments(client, action)
        case CheckBannedInChannel():
            return await _dispatch_check_banned(client, action)
        case CheckWriteRights():
            return await dispatch_check_write_rights(client, action)
        case ResolveChat():
            return await dispatch_resolve_chat(client, action)
        case ReadChatMessages():
            return await dispatch_read_chat_messages(client, action)
