...
class NeurocommentSettings(BaseSettings):
    ...
    # waves (keyword sweep, global post pages, recommendation wave). The waves
    # multiply — 10 keywords x 2 pages plus 5 recommendation seeds is 36 reads on one
    # account inside a couple of minutes — so the run bounds the total instead of each
    # wave bounding itself. Spent cheapest-first; a wave cut short says so in its
    # source report rather than quietly returning less.
    ...
    discovery_max_reads_per_run: int = Field(default=24, ge=1, le=200)
