...
// Переписка аккаунтов: СЕМЬ пар, и число здесь работающее, а не декоративное.
// Карточка теперь список пар с потолком 420px, а строка — 63px: на семи парах
// список обязан ПРОКРУЧИВАТЬСЯ. Ровно этим снимок и стережёт дефект, который
// иначе виден только на живом стенде: строка с `overflow-hidden` теряет свой
// автоминимум, и без запрета сжатия строки жались в потолок, обрезая себе
// вторую строку с последней репликой. На двух парах (потолок не достигнут)
// гейт этого не видел.
//
// Единственная фикстура в этом файле, которая НЕ берёт `NOW`, и это не
// небрежность: строка пары печатает срок («вчера», «3 дн.»), а он считается от
// текущих суток. С замороженной датой снимок расходился бы с эталоном каждые
// сутки — гейт краснел бы сам по себе. Относительные метки дают тот же текст в
// любой день; поэтому у каждой пары свежесть либо «сейчас», либо целые сутки —
// время суток в метке сделало бы снимок разным каждую минуту.
const SECONDS_AGO = new Date(Date.now() - 10_000).toISOString();
const DAY_MS = 24 * 60 * 60 * 1000;
const daysBack = (days: number) => new Date(Date.now() - days * DAY_MS).toISOString();
...
export const warmingDialogues = {
  messages: [
    dialogue(1, 0, 'Да, с утра. Скинь повестку, если есть', SECONDS_AGO),
    dialogue(0, 1, 'Ты завтра на созвоне?', SECONDS_AGO),
    dialogue(0, 1, 't.me/joinchat/AAAAAE7f2kQwertyuiop', SECONDS_AGO),
    dialogue(2, 1, 'Ок, вечером посмотрю', daysBack(1)),
    dialogue(1, 2, 'Глянь последний пост в канале', daysBack(1)),
    dialogue(3, 0, 'Почти закрыл, остались тесты', daysBack(3)),
    dialogue(2, 3, 'Держи файл, посмотри когда будет время', daysBack(4)),
    dialogue(4, 0, 'Договорились', daysBack(6)),
    dialogue(4, 1, 'До связи', daysBack(9)),
    dialogue(3, 4, 'Ага, подписался утром', daysBack(12)),
  ],
};
