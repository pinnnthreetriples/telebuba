...
// Поля, которые обрезали своё значение ЕЩЁ ДО этой ветки. Проверено экспериментом:
// с возвращёнными старыми значениями `fontSize.body` и ритма обрезался только этот вход,
// а поля паузы — нет. Значит он не регрессия отсюда, и починка его — отдельная работа:
// у поля правильные `min-w-0 flex-1`, но ширина содержимого приходит нулевой, и причина
// не в дизайн-системе.
const KNOWN_CLIPPED: Record<string, string[]> = {
  neuroshilling: ['Персона роли 1', 'Персона роли 2'],
};
