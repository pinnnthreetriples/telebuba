...
const ROLE = {
  ...
  canvas: 'Фон страницы, рельс под плашкой',
  track: 'Всё заполняемое: дорожка прогресса, чип, счётчик',
  scrim:
  ...
  'primary-deep': 'Синий для мелкого текста на tint и wash: основной даёт там 4.38:1',
...
const SWATCH_GROUPS = [
  {
    title: 'Основа',
    keys: ['canvas', 'scrim', 'surface', 'track'],
    prose:
      'Тон отвечает на вопрос «где я нахожусь»: белое — предмет, <strong>surface</strong> — вложено в предмет, <strong>canvas</strong> — под предметом, <strong>track</strong> — то, что заполняется. <strong>scrim</strong> стоит особняком: это не тон основы, а завеса поверх фотографии, единственное полупрозрачное значение в наборе.',
  },
  {
    title: 'Текст',
    keys: ['ink'],
    prose:
      '<strong>muted</strong> и <strong>subtle</strong> — два серых, которыми набран мелкий текст, и оба стоят на пороге AA, а не там, где выглядели лучше: <strong>muted</strong> давал 4.10:1 на <code>track</code>, <strong>subtle</strong> — 2.88:1 на белом. Запас между ними и <strong>body</strong> сжат намеренно; другой выход — ступень, про которую система знает, что её не прочесть.',
  },
