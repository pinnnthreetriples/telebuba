...
// Ни одна тень и ни одна завеса не держит краску литералом.
//
// Гейт на дефект, который выглядел исправленным: каналы появились ради кейфреймов,
// `shadow.focus` перешёл на них сразу, а остальные пять теней и обе завесы остались с
// `rgba(11,11,12,…)`, `rgba(0,0,0,…)` и `rgba(0,102,255,0.3)` — то есть с третьей и
// четвёртой копией краски, которую палитра уже хранит. Перекрасив `blue600`, пилюля табов
// осталась бы прежней, и НИЧЕГО бы об этом не сказало: тень с правильным цветом и тень с
// копией правильного цвета выглядят одинаково до дня перекраски.
//
// Читается исходник, а не значения, по той же причине, что и у семантического гейта ниже:
// у копии значение совпадает по определению, и сверка значений слепа именно здесь.
// Комментарии снимаются — иначе проза, называющая прежние литералы (в том числе абзац
// выше), сама стала бы нарушением.
test('краска теней и завес приходит только из канала', () => {
  const source = readFileSync(join(ROOT, 'tokens', 'primitives.ts'), 'utf8');
  const COLOUR = /\brgba?\(/;
  const FROM_CHANNEL = /\brgb\(\$\{channel\.\w+\} \/ [\d.]+\)/;

  const offenders = source
    .split('\n')
    .map((line, at) => ({ code: line.replace(/\/\/.*$/, ''), at: at + 1 }))
    .filter(({ code }) => COLOUR.test(code))
    .filter(({ code }) => !FROM_CHANNEL.test(code))
    .map(({ code, at }) => `${String(at)}: ${code.trim()}`);

  // Если разбор перестанет находить краску, утверждение ниже выполнится на пустом
  // списке — а таких строк в файле восемь.
  expect(source.split(COLOUR).length - 1).toBeGreaterThan(5);
  expect(offenders).toEqual([]);
});
