...
export function HintBubble({ text, example }: { text: string; example?: string }) {
  return (
    ...
    <span
      className="pointer-events-none absolute left-1/2 top-[calc(100%+7px)] z-20 hidden w-[230px] -translate-x-1/2 rounded-[10px] border border-line bg-white p-[10px] text-left text-[11.5px] leading-snug text-ink-muted shadow-[0_6px_20px_rgba(0,0,0,0.12)] group-hover:block group-focus-within:block"
      role="tooltip"
    >
      {text}
