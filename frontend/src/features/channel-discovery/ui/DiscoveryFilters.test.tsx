...
describe('DiscoveryFilters', () => {
  ...
  it('flags a limit outside the bounds instead of clamping it', async () => {
    const { last } = renderFilters();
    const limit = screen.getByRole('spinbutton', { name: 'Лимит результатов' });
    ...
    expect(screen.getByText('Целое число от 1 до 500')).toBeInTheDocument();
  ...
  it('marks both subscriber bounds when they are the wrong way round', () => {
    ...
    expect(screen.getByRole('spinbutton', { name: 'Подписчиков от' })).toHaveAttribute(
      'aria-invalid',
      'true',
    );
    expect(screen.getByRole('spinbutton', { name: 'Подписчиков до' })).toHaveAttribute(
      'aria-invalid',
      'true',
    );
  ...
  it('writes the subscriber bounds and the seed channel', async () => {
    ...
    await userEvent.type(screen.getByRole('spinbutton', { name: 'Подписчиков от' }), '10');
    expect(last()?.minSubscribers).toBe('10');
    await userEvent.type(screen.getByRole('spinbutton', { name: 'Подписчиков до' }), '50');
