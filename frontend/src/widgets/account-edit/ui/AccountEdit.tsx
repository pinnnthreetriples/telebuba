...
import { FeedbackMark, FormField, Modal } from '@/shared/ui';
...
export function AccountEdit({ account, onBack }: { account: AccountRead; onBack: () => void }) {
  ...
  // alive; otherwise the matching health colour + a session-scoped inactive
  // label (distinct from the hero status badge, which carries the status code).
  const health = accountHealth(account.status);
  const sessionDot = HEALTH_DOT[health];
  const sessionText =
    account.status === 'alive' ? t('accounts.edit.sessionOk') : t('accounts.edit.sessionInactive');

  // Real proxy-connection row: unassigned / failed / connected, driven by the
  // stored proxy_id + proxy_status.
      ...
      <div className="mb-[14px] grid grid-cols-2 gap-[14px]">
        ...
        <Section title={t('accounts.edit.proxy')}>
          ...
          {unassignProxy.isError ? (
            <div className="mb-3 text-[11.5px] text-[#c0473f]">
