...
def _results() -> str:
    return (
        ...
        "    schemas.beta.x_Model____validate__mutmut_1: killed"
    )
...
def _write_inputs(tmp_path: Path) -> tuple[Path, Path, Path, Path]:
    ...
    project.write_text('[tool.mutmut]\nsource_paths = ["services", "schemas"]\n', encoding="utf-8")
...
def test_baseline_integrity_rejects_scope_mismatch(
    ...
    with pytest.raises(mutation_report.ReportError, match="does not match project scope"):
