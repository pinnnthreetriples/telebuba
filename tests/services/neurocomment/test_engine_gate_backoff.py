...
The pause only ran when a pending challenge resolved to failed, and that channel
...
from core.db import fetch_channel_paused_until, list_recent_logs
...
@pytest.mark.asyncio
async def test_gate_without_pending_challenge_pauses_the_channel(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    # K=1: one account can only be gated once (the gate marks its pair not-ready), so
    # the counter reaches K across accounts in production — the rounds themselves are
    # covered by test_channel_pause. What matters here is that a gate counts at all.
    ...
    # The row names the account that tripped it — the feed is read one line per
    # account action, and an unattributed row is unactionable.
    paused = next(
        entry
        for entry in await list_recent_logs(limit=50)
        if entry.event == "neurocomment_channel_paused"
    )
    assert paused.account_id == "acc-1"
    assert paused.extra["rounds"] == 1
    # The counter is per-channel, but the row names the account that ended the round —
    # the feed is read one line per account action, and an unattributed row is unactionable.
    assert paused.account_id == "acc-1"
...
async def test_delivered_comment_resets_the_window_without_a_challenge(
    ...
    per-account gates would accumulate with no decay and pause a working channel.
    """
    ...
    # The next gate is the 1st of a fresh window, not the 2nd — no round ends.
