...
async def test_campaign_comments_are_bucketed_to_their_own_channels(
    ...
    campaigns = AsyncMock(
        return_value={
            "@a": SimpleNamespace(campaign_id="campaign"),
            "@b": SimpleNamespace(campaign_id="campaign"),
        }
    )
    monkeypatch.setattr(_sweep, "fetch_active_campaigns_for_channels", campaigns)
    listed = AsyncMock(
    ...
    campaigns.assert_awaited_once_with(["@a", "@b", "@orphan"])
...
async def test_one_campaign_read_failure_does_not_touch_other_campaigns(
    ...
    campaigns = AsyncMock(
        return_value={
            "@a": SimpleNamespace(campaign_id="@a"),
            "@b": SimpleNamespace(campaign_id="@b"),
        }
    )
    monkeypatch.setattr(_sweep, "fetch_active_campaigns_for_channels", campaigns)
