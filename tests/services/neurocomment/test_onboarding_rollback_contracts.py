...
from services.neurocomment import _onboard_channel, _onboard_pair, _seams, onboarding
...
@pytest.mark.asyncio
async def test_channel_pause_skips_join_without_overwriting_pair_state(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(_onboard_pair, "fetch_readiness", AsyncMock(return_value=None))
    monkeypatch.setattr(
        _onboard_pair,
        "fetch_channel_paused_until",
        AsyncMock(return_value="2999-01-01T00:00:00+00:00"),
    )
    execute = AsyncMock()
