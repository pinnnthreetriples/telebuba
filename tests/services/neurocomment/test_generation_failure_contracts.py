...
@pytest.mark.asyncio
async def test_generation_exhaustion_fails_claim_without_delay_or_gateway(
    ...
    await _generate._generate_and_post(_event(), _campaign(), "account", _limits())
...
@pytest.mark.parametrize(
    ...
    [("delay", RuntimeError, "delay"), ("gateway", ConnectionError, "down")],
)
async def test_every_pre_delivery_failure_releases_exact_and_semantic_claims(
    ...
    sleep_error = RuntimeError("delay") if failure_stage == "delay" else None
    monkeypatch.setattr(_generate.asyncio, "sleep", AsyncMock(side_effect=sleep_error))
