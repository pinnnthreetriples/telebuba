...
from services.neurocomment import _classify, _seams, onboarding
...
async def test_retry_statuses_are_non_terminal_without_readiness_write(
    ...
    write = AsyncMock()
    audit = AsyncMock()
    monkeypatch.setattr(_classify, "upsert_readiness", write)
    monkeypatch.setattr(_classify, "log_event", audit)
...
async def test_terminal_join_failures_persist_distinct_sentinels(
    ...
    expected: dict[str, object] = {
        "joined": joined,
        "captcha_passed": captcha,
        "ready": False,
    }
    if error_type == "SomeFailure":
        expected["access_lost_reason"] = error_type
    write.assert_awaited_once_with("account", "@channel", **expected)
...
async def test_hard_ban_is_persisted_as_sticky_banned_state(
    ...
    write = AsyncMock()
    confirm_ban = AsyncMock(return_value=True)
    monkeypatch.setattr(_classify, "upsert_readiness", write)
    monkeypatch.setattr(_classify.bans, "confirm_group_ban_and_leave", confirm_ban)
    ...
    confirm_ban.assert_awaited_once_with("account", "@channel")
...
async def test_solver_outcomes_map_to_readiness(
    ...
    monkeypatch.setattr(
        _classify.challenge, "solve_if_present", AsyncMock(return_value=solver_outcome)
    )
    write = AsyncMock()
    monkeypatch.setattr(_classify, "upsert_readiness", write)
...
async def test_public_solver_disabled_join_persists_fully_ready_pair(
    ...
    monkeypatch.setattr(_seams, "execute_read", read.execute_read)
    monkeypatch.setattr(_seams, "execute", join.execute)
    monkeypatch.setattr(_classify.challenge, "solve_if_present", solver)
...
async def test_public_campaign_solver_override_controls_solver_execution(
    ...
    result = await asyncio.wait_for(onboarding.onboard_campaign(campaign_id), timeout=2.0)
