...
@pytest.mark.asyncio
async def test_success_preserves_live_cooldown_and_records_exact_commit(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    now = datetime.now(UTC)
    await _state.set_cooldown("account", now + timedelta(minutes=5), "@channel")
    posted = AsyncMock()
    ...
    assert _state.in_cooldown("account", now, "@channel") is True
...
async def test_wait_families_fail_release_and_apply_correct_scope(
    ...
    cooldown.assert_awaited_once_with("account", 30, scope)
    log.assert_awaited_once_with(
        "WARNING",
