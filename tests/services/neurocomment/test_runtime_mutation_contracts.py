...
from services.neurocomment import _lifecycle, _runtime
...
async def test_stale_claim_reclaim_uses_configured_cutoff_and_logs_nonzero_count(
    ...
    audit = AsyncMock()
    monkeypatch.setattr(_lifecycle, "datetime", _FrozenDateTime)
    monkeypatch.setattr(_lifecycle, "reclaim_stale_claims", reclaim)
    monkeypatch.setattr(_lifecycle, "log_event", audit)
...
async def test_stale_claim_reclaim_does_not_emit_empty_activity(
    ...
    audit = AsyncMock()
    monkeypatch.setattr(_lifecycle, "reclaim_stale_claims", AsyncMock(return_value=0))
    monkeypatch.setattr(_lifecycle, "log_event", audit)
