...
from services.warming import _seams, _steps
...
def no_human_pause(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(_steps, "_human_pause", no_pause)
...
async def test_reaction_gate_does_not_consume_entropy_when_reaction_is_impossible(
    ...
    tally = _steps._ChannelTally()
    outcome = await _steps._read_and_react(
        "acc-1",
        "channel_one",
        tally,
    ...
    assert (outcome.reads, outcome.reactions, outcome.failures, tally.attempts, flood_status) == (
        expected
    )
...
async def test_reaction_gate_draws_exactly_once_per_eligible_read(
    ...
    assert tally.attempts == len(expected_actions)
