...
@pytest.mark.asyncio
async def test_tdata_rollback_keeps_credentials_after_database_delete_failure(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    """A failed row delete keeps its credential while later rollbacks still run."""
    ...
    monkeypatch.setattr("services.accounts._import_rollback.delete_account", unavailable_database)
    ...
    assert (settings.telegram.session_dir / f"{first_id}.session").exists()
    assert (settings.telegram.session_dir / f"{second_id}.session").exists()
