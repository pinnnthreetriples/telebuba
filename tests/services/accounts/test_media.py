...
async def test_remove_account_story_executes_action_and_invalidates_cache(
    ...
    invalidated: list[str] = []
    audit = AsyncMock()
    account_id = "account-story-remove"
    audit_extra = {"story_id": 9876}
    audit_event = "account_story_removed"
    ...
    monkeypatch.setattr("services.accounts.media.execute", fake_execute)
    monkeypatch.setattr("services.accounts.media.log_event", audit)
    monkeypatch.setattr(
        "services.accounts.media.invalidate_account_profile_cache",
        invalidated.append,
    )
    result = await remove_account_story(
        AccountStoryRemove(account_id=account_id, story_id=9876),
    )
    assert (result.status, invalidated) == ("ok", [account_id])
    assert isinstance(captured[0], RemoveStory)
    assert captured[0].story_id == 9876
    audit.assert_awaited_once_with("INFO", audit_event, account_id=account_id, extra=audit_extra)
...
async def test_set_account_story_pinned_executes_action_and_invalidates_cache(
    ...
    *,
    ...
    invalidated: list[str] = []
    audit = AsyncMock()
    account_id = "account-story-pin"
    audit_extra = {"story_id": 3210, "pinned": pinned}
    audit_event = "account_story_pinned"
    ...
    result = await set_account_story_pinned(
        AccountStoryPin(account_id=account_id, story_id=3210, pinned=pinned),
    )
    assert (result.status, invalidated) == ("ok", [account_id])
