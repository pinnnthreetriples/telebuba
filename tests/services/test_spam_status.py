...
async def test_refresh_cache_boundaries(
    ...
    async def account_exists(_account_id: str) -> object:
        return object()
    ...
    monkeypatch.setattr(spam_status, "fetch_account", account_exists)
...
async def test_refreshes_for_different_accounts_do_not_block_each_other(
    ...
    await create_account(AccountCreate(account_id="independent-a"))
    await create_account(AccountCreate(account_id="independent-b"))
...
async def test_refresh_never_probes_an_account_with_no_row(
    ...
    """An unknown account must not mint a Telethon session as a probe side effect."""
    probed: list[str] = []
...
async def test_refresh_does_not_cache_a_probe_error(monkeypatch: pytest.MonkeyPatch) -> None:
    """A probe that never reached @SpamBot is not a cacheable account reading."""
    await create_account(AccountCreate(account_id="acc-1"))
