...
from features.shared.nav import NAV_LINKS, nav_link_classes


def test_nav_links_cover_every_registered_page() -> None:
    routes = {route for _, route in NAV_LINKS}
    assert {"/", "/warming", "/neurocomment", "/logs"} <= routes
...
def test_active_route_is_emphasised() -> None:
    active = nav_link_classes("/neurocomment", "/neurocomment")
    inactive = nav_link_classes("/", "/neurocomment")
    assert "font-medium" in active
    assert "font-medium" not in inactive
